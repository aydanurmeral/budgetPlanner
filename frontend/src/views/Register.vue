    ...
    <div class="w-full lg:w-1/2 flex items-center justify-center p-6">
      <div class="w-full max-w-md">
        <div class="text-center mb-8">
          ...
          <p class="text-gray-500 dark:text-gray-400">Finans takip uygulamasına katılın</p>
        </div>
...
const handleRegister = async () => {
  ...
  try {
    ...
    error.value = 'Kayıt başarılı ancak beklenmeyen bir yanıt alındı'
  } catch (err) {
    console.error('Register error:', err)
    
    if (err.response) {
      error.value = err.response.data?.error || err.response.data?.message || 'Kayıt sırasında bir hata oluştu'
      
      if (err.response.status === 201 || (err.response.data && err.response.data.token)) {
      ...
      error.value = 'Sunucuya bağlanılamadı. Lütfen tekrar deneyin.'
    } else {
      error.value = 'Kayıt sırasında bir hata oluştu: ' + (err.message || 'Bilinmeyen hata')
    }
  } finally {
    loading.value = false
  }
